...
if __name__=='__main__':
	...
	parser.add_argument('--file', default='../data/data_2nd.csv', help='observations in .csv format')
	...
	parser.add_argument('--alpha', type=float, default=1e-2, help='alpha for GP regression model (observation variance)')
	...
	parser.add_argument('--csv', help='file name to save --show result in csv format. Use with --show option. Example: "--show --csv SAVE_HERE.csv"')
	...
	parser.add_argument('--verbose', action='store_true', help='print intermediate results of two-step search.')
	# target value & prior mean
	parser.add_argument('--target', default='0.1404,0.3065', type=str)
	...
	if args.ngrid is None:
		...
		if args.show and (D - len(l_fix)==1):
			...
			# _x_md = X_nd[data_idx_m>0,:][:,dim] # raw scale
			...
			plt.plot(next_d[0], next_d[1], marker='P', mew=0, markersize=12, color=(.4,1.,.9), markeredgecolor=(.2,.8,.7), alpha=.7)
			plt.colorbar()
			plt.title('prediction x')
			plt.subplot(2,2,4)
